...
public class RedisConfiguration {
	...
	public PropertiesWrap getRedisPropertiesWrap() throws IOException {
		Properties pro = null;
		if (StringUtils.isBlank(location)) {
			logger.warn("未设置redis配置，尝试使用默认配置");
			pro = new Properties();
			pro.setProperty("redis.hostAndPort", "127.0.0.1:6379");
		} else {
			pro = PropertiesLoaderUtils.loadProperties(new ClassPathResource(location));
		}
	...
	public JedisConnectionFactory getJedisConnectionFactory(@Qualifier("redisPropertiesWrap") PropertiesWrap config) throws IOException{
		...
		if (StringUtils.isBlank(hostAndPortStr)) {
			logger.error("redis配置获取[hostAndPortStr]失败，请检查");
			throw new IllegalArgumentException("redis配置获取[hostAndPortStr]失败，请检查");
		}
